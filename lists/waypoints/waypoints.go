...
// Geo-points set organized as a Singly Linked List.
// Not thread-safe.
...
func (list *Waypoints) PushBack(p *Point) *Node {
	defer func() {
		list.size++
	}()
...
func (list *Waypoints) PushFront(p *Point) *Node {
	defer func() {
		list.size++
	}()
...
func (list *Waypoints) InsertAfter(p *Point, mark *Node) *Node {
	var e *Node
...
func (list *Waypoints) Remove(n *Node) *Point {
	if list.head == nil || list.size == 0 {
		return nil
	}
...
func (list *Waypoints) Front() *Node {
	return list.head
}

func (list *Waypoints) Len() int {
	return list.size
}

func (list *Waypoints) PrintAll() error {
	fmt.Printf("Current route is (%d point(s)):\n", list.Len())

	if list.Front() == nil {
		return errors.New("Waypoints are empty")
	}
